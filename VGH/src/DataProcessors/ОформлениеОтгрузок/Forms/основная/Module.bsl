...
&НаКлиенте
Процедура СписокРеестров(Команда)
	
	ПараметрыФормы=Новый Структура;
	ПараметрыФормы.Вставить("ТекущееУстройство",Объект.ТекущееУстройство);
	ПараметрыФормы.Вставить("ТекущееРабочееМесто",Объект.ТекущееРабочееМесто);
	ОткрытьФорму("Документ.РеестрОтгрузок.ФормаСписка",ПараметрыФормы,ЭтотОбъект,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
