Функция ПолучитьHTTPСоединение(ПараметрыПодключения,СтруктураОтвета)
	...
	КонецЕсли;
	Попытка		
		Если ЗначениеЗаполнено(перПорт1) И перПорт1 <= 0 Тогда 
			HTTPСоединение = Новый HTTPСоединение(перСервер,,перПользователь,перПароль,,,ssl,Ложь);
		Иначе 
			HTTPСоединение = Новый HTTPСоединение(перСервер,перПорт1,перПользователь,перПароль,,,ssl,Ложь);
		КонецЕсли;	
	Исключение
...
Процедура ПолучитьРезультатНаСервере(МетодСервиса,вхПараметры,СтруктураОтвета,MP = Ложь) Экспорт //***Д***//MP = Ложь
	...
	Иначе
		Если Не (СтрНайти(вхПараметры.ПараметрыЗапроса.order, "-") <> 0 И СтрДлина(вхПараметры.ПараметрыЗапроса.order) = 13) Тогда
			Сайт_ПараметрыПодключения=ПланыОбмена.ОбменсСайтом.ПолучитьПараметрыАктивногоСайта(); //ПолучитьПараметрыподключения("Mangento");
		Иначе
			Сайт_ПараметрыПодключения=ПланыОбмена.ОбменсСайтом.ПолучитьПараметрыАктивногоСайта(Истина); //ПолучитьПараметрыподключения("Розница1С");
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьДанныеТоваровНаСервере(Ссылка)  экспорт
	//мОбъект=Форма.РеквизитФормыВЗначение("Объект");
	мОбъект=Ссылка.ПолучитьОбъект();
	мДанные = Новый Массив();

	//мм=ОбщегоНазначенияКлиентСервер.ТаблицаЗначенийВМассив(мОбъект.ГрузовыеМеста);
	тз= мОбъект.Товары.Выгрузить(,"sku,title,qty");
	
	мДанные=ОбщегоНазначения.ТаблицаЗначенийВМассив(тз);
	Возврат мданные;
	
КонецФункции // ()
Функция ПолучитьФайлССервера(ФайлНаВебСервере, MP = ложь, УникальныйИдентификатор, пЭтоВеб = Ложь, НомерДокумента = "") Экспорт
	...
	Если СтрНайти(НомерДокумента, "-") <> 0 И СтрДлина(НомерДокумента) = 13 Тогда
		...
		Если ВРег(URLРазделенный.Протокол)="HTTPS" Тогда
	    	ssl = Новый ЗащищенноеСоединениеOpenSSL();
	    Иначе	
	    	ssl = Неопределено;
		КонецЕсли;
		Попытка
			HTTPСоединение = Новый HTTPСоединение(URLРазделенный.ИмяСервера,,,,,,ssl,Ложь);
	    Исключение
	    	//Подчищаем соединение
	    	HTTPСоединение = Неопределено;
	    	Возврат ложь;
		...
		HTTPОтвет = ПолучитьHTTPОтвет(URLРазделенный.ПутьКФайлуНаСервере, ИмяФайла, HTTPСоединение, Заголовки);
