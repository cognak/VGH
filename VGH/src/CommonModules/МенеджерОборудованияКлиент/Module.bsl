...
Процедура ВыполнитьНастройкуОборудования(Идентификатор, ОповещениеПриЗавершении = Неопределено) Экспорт
	...
	ОбработчикДрайвера = МенеджерОборудованияКлиентПовтИсп.ПолучитьОбработчикДрайвера(ДанныеУстройства.ОбработчикДрайвера, ДанныеУстройства.ВСоставеКонфигурации, ДанныеУстройства.ТипОборудованияИмя);
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Произошла ошибка инициализации формы настройки драйвера.';uk='Відбулася помилка ініціалізації форми налаштування драйвера.'")); 
	КонецЕсли;
...
Процедура ВыполнитьНастройкуОборудования_Завершение(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Иначе
			СообщениеОбОшибке = НСтр("ru='Не удалось сохранить параметры устройства.';uk='Не вдалося зберегти параметри обладнання.'");
...
Процедура ПредложитьВыбратьУстройство(ОповещениеВыбора, ТипОборудования, ТекстЗаголовкаВыбора, 
	...
	Если Не ОбновитьРабочееМестоКлиента() Тогда
		ТекстСообщения = НСтр("ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';uk='Попередньо необхідно вибрати робоче місце торгового обладнання поточного сеансу.'");
...
Процедура НачатьВыборФайлаДрайвера(ОповещениеПриВыборе) Экспорт 
	...
	ДиалогОткрытияФайла.Фильтр = НСтр("ru='Файл драйвера';uk='Файл драйвера'") + ?(МенеджерОборудованияКлиентПовтИсп.ЭтоLinuxКлиент(), "(*.zip)|*.zip", "(*.zip, *.exe)| *.zip; *.exe");  
...
Процедура НачатьПодключениеОборудования(ОповещениеПриПодключении, ИдентификаторКлиента, ТипыПО = Неопределено, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если Не Результат Тогда
		Если ОповещениеПриПодключении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';uk='Попередньо необхідно вибрати робоче місце торгового обладнання поточного сеансу.'");
		...
		Для Каждого Устройство Из СписокОборудования Цикл
			...
			Если ПодключенноеУстройство = Неопределено Тогда // Если устройство не было подключено ранее.
				...
				Если ОбработчикДрайвераМодуль = Неопределено Тогда
					...
					Если ОповещениеПриПодключении <> Неопределено Тогда
						ОписаниеОшибки = НСтр("ru='Не удалось подключить обработчик драйвера.';uk='Не вдалося підключити оброблювач драйвера.'");
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.';uk='%Наименование%: Не вдалося завантажити драйвер пристрою."
"Перевірте, що драйвер коректно встановлений і зареєстрований у системі.'");
						...
						Иначе
							...
							Иначе
								...
								Если ОповещениеПриПодключении <> Неопределено Тогда
									ОписаниеОшибки = НСтр("ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';uk='Не вдалося підключити обладнання ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)'");
									ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , НовоеПодключение.Наименование);
	...
	ИначеЕсли ИдентификаторУстройства <> Неопределено И ОповещениеПриПодключении <> Неопределено Тогда
		ОписаниеОшибки =  НСтр("ru='Выбранное устройство не может использоваться для подключения. Укажите другое устройство.';uk='Обране обладнання не може використовуватися для підключення. Вкажіть інше обладнання.'");
		РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		ВыполнитьОбработкуОповещения(ОповещениеПриПодключении, РезультатВыполнения);
...
Процедура НачатьПодключениеОборудования_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		// Сообщим пользователю о том, что не удалось подключить устройство.
		Если Параметры.ОповещениеПриПодключении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки%';uk='Не вдалося підключити обладнання ""%Наименование%"": %ОписаниеОшибки%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , Параметры.НовоеПодключение.Наименование);
						...
						Иначе
							...
							Иначе
								...
								Если НЕ Результат Тогда
									// Сообщим пользователю о том, что не удалось подключить устройство.
									Если ОповещениеПриОтключении <> Неопределено Тогда
										ОписаниеОшибки = НСтр("ru='При отключении устройства ""%Наименование%"" произошла ошибка: %ОписаниеОшибки%';uk='При відключенні обладнання ""%Наименование%"" відбулася помилка: %ОписаниеОшибки%'");
										ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , ПодключенноеУстройство.Наименование);
										ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
										РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
										ВыполнитьОбработкуОповещения(ОповещениеПриОтключении, РезультатВыполнения);
...
Процедура НачатьОтключениеОборудование_Завершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		// Сообщим пользователю о том, что не удалось подключить устройство.
		Если Параметры.ОповещениеПриОтключении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='При отключении устройства ""%Наименование%"" произошла ошибка.';uk='При відключенні обладнання ""%Наименование%"" відбулася помилка.'");
...
Процедура НачатьОтключениеВсегоОборудования(ОповещениеПриОтключении = Неопределено) Экспорт
	...
	Для Каждого ПодключенноеУстройство Из глПодключаемоеОборудование.ПараметрыПодключенияПО Цикл
		...
		Если ОбъектДрайвераЭтоCOM(ПодключенноеУстройство.ОбъектДрайвера) Тогда
			ПодключенноеУстройство.ОбъектДрайвера = ПолучитьОбъектДрайвераДляCOM(ПодключенноеУстройство);
		КонецЕсли;
...
Процедура НачатьОтключениеВсегоОборудованияЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ru='При отключении оборудования произошла ошибка: ""%ОписаниеОшибки%"".';uk='При відключенні обладнання відбулася помилка: ""%ОписаниеОшибки%"".'");
...
"Перевірте, що драйвер коректно встановлений і зареєстрований у системі.'");
				...
				Если ОбработчикДрайвераМодуль = Неопределено Тогда
					// Сообщить об ошибке, что не удалось загрузить драйвер.
					ОписаниеОшибки = ОписаниеОшибки +  НСтр("ru='Не удалось подключить обработчик драйвера.';uk='Не вдалося підключити оброблювач драйвера.'");
					КонечныйРезультат = Ложь;
					Продолжить;
				Иначе
				...
				Иначе
					// Сообщим пользователю о том, что не удалось подключить устройство.
					ОписаниеОшибки = ОписаниеОшибки + ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС)
								   + НСтр("ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';uk='Не вдалося підключити обладнання ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)'");
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%"  , Устройство.Наименование);
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
					ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КодОшибки%"     , ВыходныеПараметры[0]);
				КонецЕсли;
...
"Укажите другое устройство.';uk='Обраний пристрій не може використовуватися для підключення."
"Укажіть інший пристрій.'");
		КонечныйРезультат = Ложь;
	ИначеЕсли ТипыПО = Неопределено И ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='Нет доступного оборудования для подключения.';uk='Немає доступного обладнання для підключення.'");
		КонечныйРезультат = Ложь;
...
"Перевірте, що драйвер коректно встановлений і зареєстрований у системі.'");
					...
					Если НЕ Результат Тогда
						ОписаниеОшибки = ОписаниеОшибки + ?(ПустаяСтрока(ОписаниеОшибки), "", Символы.ПС)
									   + НСтр("ru='При отключении устройства ""%Наименование%"" произошла ошибка: %ОписаниеОшибки% (%КодОшибки%)';uk='При відключенні обладнання ""%Наименование%"" відбулася помилка: %ОписаниеОшибки% (%КодОшибки%)'");
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%Наименование%", ПодключенноеУстройство.Наименование);
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", ВыходныеПараметры[1]);
						ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%КодОшибки%", ВыходныеПараметры[0]);
					Иначе
...
Функция ВыполнитьКоманду(Идентификатор, Команда, ВходныеПараметры, ВыходныеПараметры, Таймаут = -1) Экспорт
	...
	Если ПодключенноеУстройство <> Неопределено Тогда
		...
		Иначе
			...
			ПараметрыПодключения.Вставить("АйПиАдресс",ВходныеПараметры[0]);
			Если ОбработчикДрайвераМодуль = Неопределено Тогда
				ТекстСообщения = НСтр("ru='Не удалось подключить обработчик драйвера.';uk='Не вдалося підключити оброблювач драйвера.'");
				ВыходныеПараметры = Новый Массив();
				ВыходныеПараметры.Добавить(999);
	...
	Иначе
		...
		ВыходныеПараметры = Новый Массив();
		ТекстСообщения = НСтр("ru='Устройство не подключено. Перед выполнением операции устройство должно быть подключено.';uk='Пристрій не підключено. Перед виконанням операції пристрій повинен бути підключений.'");
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстСообщения);
	КонецЕсли;
...
Функция ВыполнитьДополнительнуюКоманду(Команда, ВходныеПараметры, ВыходныеПараметры, Идентификатор, Параметры) Экспорт
	...
	Иначе
		...
		ТекстСообщения = НСтр("ru='Устройство подключено. Перед выполнением операции устройство должно быть отключено.';uk='Пристрій підключено. Перед виконанням операції пристрій повинен бути відключений.'");
		ВыходныеПараметры = Новый Массив();
		ВыходныеПараметры.Добавить(999);
		ВыходныеПараметры.Добавить(ТекстСообщения);
		ВыходныеПараметры.Добавить(НСтр("ru='Установлен';uk='Встановлений'"));
...
Функция СоздатьОбъектДрайвераДляCOM(ОбъектДрайвера)
	...
	Если ОбъектДрайвера = "COM_PAntonXP_DLL.PAnton" Тогда
		Попытка
			НовыйОбъект = Новый COMОбъект("PAntonXP_DLL.PAnton");
		Исключение
			хх=ОписаниеОшибки();
		КонецПопытки;
	КонецЕсли;
...
Процедура ПодключитьОборудованиеЗавершениеПоУмолчанию(РезультатВыполнения, Параметры) Экспорт
	...
	Если НЕ РезультатВыполнения.Результат Тогда
		ТекстСообщения = НСтр("ru='При подключении оборудования произошла ошибка:""%ОписаниеОшибки%"".';uk='При підключенні обладнання відбулася помилка:""%ОписаниеОшибки%"".'");
		ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ОписаниеОшибки%" , РезультатВыполнения.ОписаниеОшибки);
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецЕсли;
...
Функция ПолучитьОбъектДрайвера(ДанныеДрайвера, ТекстОшибки = Неопределено)
	...
	Если ОбъектДрайвера = Неопределено Тогда
		Попытка
			...
			ИначеЕсли ProgID = "COM_PAntonXP_DLL.PAnton" Тогда
				Попытка
					ТестДрайвера = Новый COMОбъект("PAntonXP_DLL.PAnton");
				    ОбъектДрайвера = "COM_PAntonXP_DLL.PAnton"; // COM-соединение
				Исключение
					ОбъектДрайвера = Неопределено;
				КонецПопытки;
				
			ИначеЕсли ProgID = "COM_ArtSoft_FiscalPrinter" Тогда
				Попытка
...
Процедура УстановитьДрайвер(Идентификатор, ОповещениеИзДистрибутиваПриЗавершении = Неопределено, ОповещениеИзАрхиваПриЗавершении = Неопределено) Экспорт
	...
	Попытка  
		Если ДанныеДрайвера.ВСоставеКонфигурации Тогда
			...
			Если ДанныеДрайвера.ПоставляетсяДистрибутивом Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Данный драйвер не может быть установлен из дистрибутива.';uk='Цей драйвер не може бути встановлений з дистрибутива.'")); 
			Иначе
		...
		Иначе
			...
			Если ДанныеДрайвера.ПоставляетсяДистрибутивом Тогда
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Данный драйвер не может быть установлен и использован.';uk='Цей драйвер не може бути встановлений і використаний.'")); 
			Иначе
				СсылкаНаДрайвер = ПолучитьНавигационнуюСсылку(ДанныеДрайвера.ДрайверОборудования, "ЗагруженныйДрайвер");
	...
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Произошла ошибка при установке драйвера.';uk='Відбулася помилка при установці драйвера.'")); 
	КонецПопытки;  
...
Процедура НачатьВыполнениеКоманды(ОповещениеПриЗавершении, Идентификатор, Команда, ВходныеПараметры) Экспорт
	...
	Если ПодключенноеУстройство <> Неопределено Тогда
		...
		Если ОбработчикДрайвераМодуль = Неопределено Или ОбъектДрайвера = Неопределено Тогда
			// Сообщить об ошибке, что не удалось загрузить драйвер.
			ТекстОшибки = НСтр("ru='Не удалось подключить обработчик драйвера или загрузить драйвер.';uk='Не вдалося підключити оброблювач драйвера або завантажити драйвер.'");
	...
	Иначе
		// Сообщить об ошибке, что устройство не подключено.
		ТекстОшибки = НСтр("ru='Устройство не подключено. Перед выполнением операции устройство должно быть подключено.';uk='Пристрій не підключено. Перед виконанням операції пристрій повинен бути підключений.'");
		РезультатВыполнения = ПараметрыВыполненияОперацииНаОборудовании();
		РезультатВыполнения.Результат = Ложь;
		РезультатВыполнения.ОписаниеОшибки = ТекстОшибки; 
		ВыполнитьОбработкуОповещения(ОповещениеПриЗавершении, РезультатВыполнения);
...
Процедура НачатьВыполнениеДополнительнойКоманды(ОповещениеПриЗавершении, Команда, ВходныеПараметры, Идентификатор, Параметры) Экспорт
	...
	Иначе
		// Сообщить об ошибке, что устройство подключено.
		ТекстОшибки = НСтр("ru='Устройство подключено. Перед выполнением операции устройство должно быть отключено.';uk='Пристрій підключено. Перед виконанням операції пристрій повинен бути відключений.'");
		ВыходныеПараметры = Новый Массив();
		ВыходныеПараметры.Добавить(999);
...
"Перевірте, що драйвер коректно встановлений і зареєстрований у системі.'");
	...
	Иначе
		...
		Если ОбработчикДрайвераМодуль = Неопределено Тогда
			...
			ТекстОшибки = НСтр("ru='Не удалось подключить обработчик драйвера.';uk='Не вдалося підключити оброблювач драйвера.'");
			ВыходныеПараметры = Новый Массив();
			ВыходныеПараметры.Добавить(999);
			ВыходныеПараметры.Добавить(ТекстОшибки);
			ВыходныеПараметры.Добавить(НСтр("ru='Не установлен';uk='Не встановлений'"));
...
"Перевірте, що драйвер коректно встановлений і зареєстрований у системі.'");
				...
				Если ОбъектДрайвераЭтоCOM(Подключение.ОбъектДрайвера) Тогда
					Подключение.ОбъектДрайвера = ПолучитьОбъектДрайвераДляCOM(Подключение);
				КонецЕсли;
...
Процедура НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Фискальное устройство не выбрано или не подключено.';uk='Фіскальний пристрій не обрано або не підключено.'");
	...
	Иначе
		...
		Если ТребуетсяПроверкаАктивностиСмены Тогда
			...
			Если Не СтатусПоследнейСмены.Активна = Истина Тогда
				ОписаниеОшибки = НСтр("ru='Кассовая смена не открыта или истекла.';uk='Касова зміна не відкрита або минула.'");
				РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
				ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
				Возврат;
			Иначе
		...
		Если Параметры.ВыполняемаяКоманда = "OpenDay" Тогда
...
"Выполнить закрытие смены?';uk='На фіскальному пристрої відкрита касова зміна, дані про яку відсутні в системі."
"Виконати закриття зміни?'");
...
Процедура НачатьВыполнениеКоманды_ПослеПодтвержденияЗакрытияСмены(Результат, Параметры) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ru='На фискальном устройстве смена не закрыта';uk='На фіскальному пристрої зміна не закрита'");
...
Процедура НачатьВыполнениеКоманды_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ru='При подключении оборудования произошла ошибка: %ДополнительноеОписание%';uk='При підключенні обладнання відбулася помилка: %ДополнительноеОписание%'");
...
Процедура НачатьВыполнениеКоманды_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		ТекстСообщения = НСтр("ru='При выполнении операции произошла ошибка: %ДополнительноеОписание%';uk='При виконанні операції відбулася помилка: %ДополнительноеОписание%'");
...
Процедура НачатьФормированиеОтчетаОТекущемСостоянииРасчетов(ОповещениеПриЗавершении, УникальныйИдентификатор, ПараметрыОперации, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
			НСтр("ru='Выберите фискальное устройство';uk='Виберіть фіскальне обладнання'"),
			НСтр("ru='Фискальное устройство не подключено.';uk='Фіскальний пристрій не підключено.'"), 
			НСтр("ru='Фискальное устройство не выбрано.';uk='Фіскальний пристрій не обрано.'"),
			Истина);
	Иначе
...
Процедура НачатьЗакрытиеСменыНаФискальномУстройстве(ОповещениеПриЗавершении, УникальныйИдентификатор, ПараметрыОперации, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено, КассоваяСмена = Неопределено) Экспорт
	...
	Контекст.Вставить("УникальныйИдентификатор" , УникальныйИдентификатор);
	Контекст.Вставить("ВходныеПараметры"        , ПараметрыОперации);
	Контекст.Вставить("ВыполняемаяКоманда"      , "PrintZReport");
	...
	Если КассоваяСмена = Неопределено Тогда
		...
		Если ИдентификаторУстройства = Неопределено Тогда
			...
			ПараметрыВыбора.Вставить("Заголовок", НСтр("ru='Выберите кассовую смену для закрытия';uk='Виберіть касову зміну для закриття'"));
		...
		Иначе
			...
			Если ОписаниеПоследнейСмены = Неопределено Тогда
				Если ОповещениеПриЗавершении <> Неопределено Тогда
					ТекстСообщения = НСтр("ru='Нет кассовых смен для устройства';uk='Немає касових змін для пристрою'") + Символы.НПП + Строка(ИдентификаторУстройства);
			...
			Иначе
				...
				Иначе
					Если ОповещениеПриЗавершении <> Неопределено Тогда
						ТекстСообщения = НСтр("ru='Кассовая смена была закрыта ранее';uk='Касова зміна була закрита раніше'");
						РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ТекстСообщения, ИдентификаторУстройства);
						ВыполнитьОбработкуОповещения(ОповещениеПриЗавершении, РезультатОперации);
					КонецЕсли;
				КонецЕсли;
...
Процедура НачатьПечатьТекста(ОповещениеПриЗавершении, УникальныйИдентификатор, Текст, ИдентификаторУстройства = Неопределено, ТипОборудования = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ОписаниеОповещения = Новый ОписаниеОповещения("НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение", ЭтотОбъект, Контекст);
		ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
			НСтр("ru='Выберите печатающее устройство';uk='Виберіть друкувальний пристрій'"), 
			НСтр("ru='Печатающее устройство не подключено.';uk='Друкувальний пристрій не підключений.'"), 
			НСтр("ru='Печатающее устройство не выбрано.';uk='Друкувальний пристрій не обраний.'"), 
			Истина);
	Иначе
		НачатьВыполнениеКоманды_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Контекст);
	КонецЕсли;
		...
		Для ИндексМассива = 0 По ПозицииЧека.Количество() - 1 Цикл
			...
			ИначеЕсли ПозицияЧека.Свойство("Штрихкод") Тогда
				...
				Если НЕ ОбщийМодульОборудования.ПечатьШтрихкода(ОбъектДрайвера, Параметры, ПараметрыПодключения,
					...
					Текст = НСтр("ru='<Штрихкод %ТипШтрихКода% не распечатан>';uk='<Штрихкод %ТипШтрихКода% не роздруковано>'");
...
Функция СформироватьШаблонИнфоКвитанции(Параметры, ШиринаСтроки, ПечатьШтрихкода = Истина) Экспорт
	...
	ОрганизацияИНН = ?(НЕ ПустаяСтрока(Параметры.ИНН), НСтр("ru='ИНН:';uk='ІПН:'") + Параметры.ИНН, "");
	Если Не ПустаяСтрока(ОрганизацияИНН) Тогда
		Текст = Текст + МенеджерОборудованияКлиентСервер.ВыстроитьПоля(ОрганизацияИНН, , ШиринаСтроки) + Символы.ПС;
	КонецЕсли;  
	
	НомерКассы = ?(НЕ ПустаяСтрока(Параметры.НомерКассы), НСтр("ru='КАССА:';uk='КАСА:'") + Параметры.НомерКассы, "");
	НомерСмены = ?(НЕ ПустаяСтрока(Параметры.НомерСмены), НСтр("ru='СМЕНА:';uk='ЗМІНА:'") + Формат(Параметры.НомерСмены, "ЧГ=0"), 0);
	...
	НомерЧека = ?(НЕ ПустаяСтрока(Параметры.НомерЧека), НСтр("ru='ЧЕК:';uk='ЧЕК:'") + Параметры.НомерЧека, "");
	ДатаВремя = НСтр("ru='ДАТА:';uk='ДАТА:'") + Формат(ТекущаяДата(), "ДФ=""дд.ММ.гггг ЧЧ:мм""");
...
Процедура НачатьПодключенияЭквайринговогоТерминала(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройстваЭТ = Неопределено, ИдентификаторУстройстваПУ = Неопределено) Экспорт
	...
	Если Контекст.ИдентификаторУстройстваЭТ = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ЭквайринговыйТерминал",
			НСтр("ru='Выберите эквайринговый терминал';uk='Виберіть еквайринговий термінал'"), 
			НСтр("ru='Эквайринговый терминал не подключен.';uk='Еквайринговий термінал не підключений.'"),
			НСтр("ru='Эквайринговый терминал не выбран.';uk='Еквайринговий термінал не обраний.'"), 
			Истина);
	Иначе
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ВыбратьУстройствоЗавершение(ИдентификаторУстройстваЭТ, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваЭТ = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Эквайринговый терминал не выбран.';uk='Еквайринговий термінал не обраний.'");
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		// Ошибка подключения.
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ru='Эквайринговая операция не была произведена: %ДополнительноеОписание%';uk='Еквайрингова операція не була проведена: %ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатПодключения.ОписаниеОшибки);
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ПечатьНаТерминалеЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		...
		Иначе
			Если Параметры.ИдентификаторУстройстваПУ = Неопределено Тогда
				...
				ПредложитьВыбратьУстройство(ОписаниеОповещения, ПоддерживаемыеТипыВО,
					НСтр("ru='Выберите печатающее устройство';uk='Виберіть друкувальний пристрій'"), НСтр("ru='Печатающее устройство не подключено.';uk='Друкувальний пристрій не підключений.'"), НСтр("ru='Печатающее устройство не выбрано.';uk='Друкувальний пристрій не обраний.'"), Истина);
			Иначе
	...
	Иначе
		...
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ru='При использовании терминала произошла ошибка: %ДополнительноеОписание%';uk='При використанні термінала відбулася помилка: %ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ВыбратьПечатающегоУстройстваЗавершение(ИдентификаторУстройстваПУ, Параметры) Экспорт
	...
	Если ИдентификаторУстройстваПУ = Неопределено Тогда
		НачатьОтключениеОборудованиеПоИдентификатору(, Параметры.УникальныйИдентификатор, Параметры.ИдентификаторУстройстваЭТ);
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Печатающее устройство не выбрано.';uk='Друкувальний пристрій не обраний.'");
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ОписаниеОшибки);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
	Иначе
...
Процедура НачатьПодключенияЭквайринговогоТерминала_ПечатающееУстройствоПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		НачатьОтключениеОборудованиеПоИдентификатору(, Параметры.УникальныйИдентификатор, Параметры.ИдентификаторУстройстваЭТ);
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ru='При подключении печатающего устройство произошла ошибка: %ДополнительноеОписание%';uk='При підключенні друкувального пристрою відбулася помилка: %ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатПодключения.ОписаниеОшибки);
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
Процедура НачатьВыполнениеСверкиИтоговНаЭквайринговомТерминале_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		// Ошибка подключения.
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ТекстСообщения = НСтр("ru='При подключении терминала произошла ошибка: %ДополнительноеОписание%';uk='При підключенні термінала відбулася помилка: %ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатПодключения.ОписаниеОшибки);
			РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
...
Процедура НачатьВыполнениеОперацииНаЭквайринговомТерминале_ОшибкаПечатиЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
		Если РезультатВыполнения.Результат Тогда 
			...
			ТекстСообщения = НСтр("ru='Эквайринговая операция отменена - ошибка печати слип чека возникла ошибка: %ДополнительноеОписание%';uk='Еквайрингова операція скасована - помилка друку сліп чека виникла помилка: %ДополнительноеОписание%'");
		...
		Иначе
			...
			ТекстСообщения = НСтр("ru='Ошибка отмены операции транзакции. Обратитесь в банк. %ДополнительноеОписание%';uk='Помилка скасування операції транзакції. Зверніться до банку. %ДополнительноеОписание%'");
			ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ДополнительноеОписание%", РезультатВыполнения.ОписаниеОшибки);
		КонецЕсли;
		РезультатОперации = ПараметрыРезультатаПодключенияЭквайринговогоТерминала(Ложь, ТекстСообщения);
		ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
...
Процедура НачатьВыводИнформацииНаДисплейПокупателя(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, СтрокиТекста = Неопределено) Экспорт
	...
	ИначеЕсли ИдентификаторУстройства <> Неопределено И ОповещениеПриЗавершении <> Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='Нет подключенных дисплеев покупателя.';uk='Немає підключених дисплеїв покупця.'");
...
Процедура НачатьОперациюНаДисплейПокупателя_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ru='При использовании дисплея покупателя произошла ошибка: %ДополнительноеОписание%';uk='При використанні дисплея покупця відбулася помилка: %ДополнительноеОписание%'");
...
Процедура НачатьЗагрузкуДанныеИзТСД(ОповещениеПриЗавершении, УникальныйИдентификатор, СворачиватьДанные = Ложь) Экспорт
	...
	ПредложитьВыбратьУстройство(Оповещение, "ТерминалСбораДанных", НСтр("ru='Выберите терминал сбора данных';uk='Виберіть термінал збору даних'"), НСтр("ru='Терминал сбора данных не подключен.';uk='Термінал збору даних не підключений.'"), ,Истина);
...
"%ОписаниеОшибки%';uk='При завантаженні даних з термінала збору даних відбулася помилка."
"%ОписаниеОшибки%'");
			ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(РезультатВыполнения.Результат, ОписаниеОшибки, Параметры.ИдентификаторУстройства);  
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
...
Процедура НачатьВыгрузкуДанныеВТСД_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = ""; 
	Иначе
		ОписаниеОшибки = НСтр("ru='При выгрузке данных в терминал сбора данных произошла ошибка: %ОписаниеОшибки%';uk='При вивантаженні даних у термінал збору даних відбулася помилка: %ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьОчисткуДанныеВТСД(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(Оповещение, "ТерминалСбораДанных",
			НСтр("ru='Выберите терминал сбора данных';uk='Виберіть термінал збору даних'"), НСтр("ru='Терминал сбора данных не подключен.';uk='Термінал збору даних не підключений.'"), ,Истина);
	Иначе
...
Процедура НачатьОчисткуДанныеВТСД_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = ""; 
	Иначе
		ОписаниеОшибки = НСтр("ru='При очистке данных в терминале сбора данных произошла ошибка: %ОписаниеОшибки%';uk='При очищенні даних у терміналі збору даних відбулася помилка: %ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьПолученияВесаСЭлектронныхВесов(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ОтображатьСообщения = Ложь) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(Оповещение, "ЭлектронныеВесы",
			НСтр("ru='Выберите электронные весы';uk='Виберіть електронні ваги'"), 
			НСтр("ru='Электронные весы не подключены.';uk='Електронні ваги не підключені.'"), 
			НСтр("ru='Электронные весы не выбраны.';uk='Електронні ваги не обрані.'"),
			Истина);
	Иначе
...
Процедура НачатьПолученияВесаСЭлектронныхВесов_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='Электронные весы не выбраны.';uk='Електронні ваги не обрані.'");
...
Процедура НачатьПолученияВесаСЭлектронныхВесов_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Иначе
		Вес = Неопределено;
		ОписаниеОшибки = НСтр("ru='При использовании электронных весов произошла ошибка: %ДополнительноеОписание%';uk='При використанні електронних ваг відбулася помилка: %ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
		Если Параметры.ОтображатьСообщения Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ОписаниеОшибки);
		КонецЕсли;
...
#КонецОбласти
#Область ПроцедурыИФункцииРаботыСУстройствомВГХ
// Получает ВГХ с электронных весов.
// УникальныйИдентификатор - идентификатор формы.
// ОповещениеПриЗавершении - оповещение при завершении взвешивании и передачи веса.
//
Процедура НачатьПолученияВГХСЭлектронныхВесов(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено, ОтображатьСообщения = Ложь) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Оповещение = Новый ОписаниеОповещения("НачатьПолученияВГХСЭлектронныхВесов_ВыбратьУстройствоЗавершение", ЭтотОбъект, Контекст);
		ПредложитьВыбратьУстройство(Оповещение, "Устройства ВГХ",
			НСтр("ru='Выберите Устройство ВГХ';uk='Виберіть пристрій ВГХ'"), 
			НСтр("ru='Устройство ВГХ не подключено.';uk='Пристрій ВГХ не має з'єднання.'"), 
			НСтр("ru='Устройство ВГХ не выбрано.';uk='Пристрій ВГХ не вибрано.'"),
			Истина);
	Иначе
		НачатьПолученияВГХСЭлектронныхВесов_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Контекст);
	КонецЕсли;
	
КонецПроцедуры

Процедура НачатьПолученияВГХСЭлектронныхВесов_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Параметры.Вставить("ИдентификаторУстройства" , ИдентификаторУстройства);
	Оповещение = Новый ОписаниеОповещения("НачатьПолученияВГХСЭлектронныхВесов_ПодключениеЗавершение", ЭтотОбъект, Параметры);
	НачатьПодключениеОборудованиеПоИдентификатору(Оповещение, Параметры.УникальныйИдентификатор, ИдентификаторУстройства);
	
КонецПроцедуры
Процедура НачатьПолученияВГХСЭлектронныхВесов_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	
	Если РезультатПодключения.Результат Тогда  
		ВходныеПараметры  = Неопределено;
		Оповещение = Новый ОписаниеОповещения("НачатьПолученияВГХСЭлектронныхВесов_ВыполнитьКомандуЗавершение", ЭтотОбъект, Параметры);
		НачатьВыполнениеКоманды(Оповещение, Параметры.ИдентификаторУстройства, "ПолучитьВес", ВходныеПараметры);
	Иначе
		// Ошибка подключения весов
		ОписаниеОшибки = РезультатПодключения.ОписаниеОшибки;
...
КонецПроцедуры
Процедура НачатьПолученияВГХСЭлектронныхВесов_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
...
Процедура НачатьПечатьЭтикеток(ОповещениеПриЗавершенииПечати, УникальныйИдентификатор, ОписаниеЭтикеткиВXML, ДанныеДляПечати, ИдентификаторУстройства = Неопределено) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда	
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ПринтерЭтикеток",
			НСтр("ru='Выберите принтер этикеток';uk='Виберіть принтер етикеток'"), 
			НСтр("ru='Принтер этикеток не подключен.';uk='Принтер етикеток не підключений.'"), 
			НСтр("ru='Принтер этикеток не выбран.';uk='Принтер етикеток не обраний.'"), 
			Истина);
	Иначе
...
Процедура НачатьПечатьЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Принтер этикеток не выбран.';uk='Принтер етикеток не обраний.'");
...
"%ОписаниеОшибки%"
"Печать прервана.';uk='При обробці макета етикетки відбулася помилка."
"%ОписаниеОшибки%"
"Друк перервано.'");
...
"%ОписаниеОшибки%';uk='При підключенні обладнання відбулася помилка."
"% ОписаниеОшибки%'");
...
"%ОписаниеОшибки%';uk='При роботі з принтером етикеток відбулася помилка."
"%ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьРедактированиеМакета(ОповещениеПриЗавершенииРедактирования, ОписаниеЭтикеткиВXML, АдресХранилищаСКД) Экспорт
	...
	Исключение
		ТекстСообщения = НСтр("ru='Использование редактора этикеток невозможно! Функциональность не поддерживается.';uk='Використання редактора етикеток неможливо! Функціональність не підтримується.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
	КонецПопытки;
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикеток(ОповещениеПриЗавершении, УникальныйИдентификатор, ТаблицаВыгрузкиТоваров, ИдентификаторУстройства = Неопределено, ЧастичнаяВыгрузка = Ложь, ОтображатьСообщения = Ложь) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "ВесыСПечатьюЭтикеток",
			НСтр("ru='Выберите весы с печатью этикеток';uk='Виберіть ваги з друком етикеток'"),
			НСтр("ru='Весы с печатью этикеток не подключены.';uk='Ваги з друком етикеток не підключені.'"), 
			НСтр("ru='Весы с печатью этикеток не выбраны.';uk='Ваги з друком етикеток не вибрані.'"), 
			Истина);
	Иначе
...
Процедура НачатьВыгрузкуДанныеВВесыСПечатьюЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ТекстСообщения = НСтр("ru='Весы с печатью этикеток не выбраны.';uk='Ваги з друком етикеток не вибрані.'");
...
"%ОписаниеОшибки%';uk='При вивантаженні даних в обладнання відбулася помилка."
"% ОписаниеОшибки%'");
...
Процедура НачатьОчисткуТоваровВВесахСПечатьюЭтикеток_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		Если Параметры.СледующееОповещение <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Весы с печатью этикеток не выбраны.';uk='Ваги з друком етикеток не вибрані.'");
...
Процедура НачатьОчисткуТоваровВВесахСПечатьюЭтикеток_ПодключениеЗавершение(РезультатПодключения, Параметры) Экспорт
	...
	Иначе
		ОписаниеОшибки = НСтр("ru='При подключении устройства произошла ошибка. |%ОписаниеОшибки%';uk='При підключенні обладнання відбулася помилка. |%ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатПодключения.ОписаниеОшибки);
...
Процедура НачатьОчисткуТоваровВВесахСПечатьюЭтикеток_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = НСтр("ru='Ошибок нет.';uk='Помилок немає.'");
	Иначе
		ОписаниеОшибки = НСтр("ru='При очистке данных в оборудование произошла ошибка. |%ОписаниеОшибки%';uk='При очищенні даних в обладнанні відбулася помилка. |%ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьОткрытиеСессииСчитывателяRFID(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства = Неопределено) Экспорт
	...
	ИначеЕсли  ОповещениеПриЗавершении <> Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='Нет подключенных RFID считывателей.';uk='Немає підключених RFID зчитувачів.'");
		РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
		ВыполнитьОбработкуОповещения(ОповещениеПриЗавершении, РезультатОперации);
	КонецЕсли;
...
Процедура НачатьОткрытиеСессииСчитывателяRFID_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = "";
	Иначе
		ОписаниеОшибки = НСтр("ru='При работе со считывателем RFID произошла ошибка: %ДополнительноеОписание%';uk='При роботі зі зчитувачем RFID відбулася помилка: %ДополнительноеОписание%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ДополнительноеОписание%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
...
Процедура НачатьЗаписьДанныхВМеткуRFID(ОповещениеПриЗавершении, УникальныйИдентификатор, ИдентификаторУстройства, ПараметрыЗаписи, Таймаут = 0) Экспорт 
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		...
		ПредложитьВыбратьУстройство(ОписаниеОповещения, "СчитывательRFID",
			НСтр("ru='Выберите RFID считыватель';uk='Виберіть RFID зчитувач'"), НСтр("ru='Нет подключенных RFID считывателей.';uk='Немає підключених RFID зчитувачів.'"));
	Иначе
...
Процедура НачатьЗаписьДанныхВМеткуRFID_ВыбратьУстройствоЗавершение(ИдентификаторУстройства, Параметры) Экспорт
	...
	Если ИдентификаторУстройства = Неопределено Тогда
		ОписаниеОшибки = НСтр("ru='RFID считыватель  не подключен.';uk='RFID зчитувач не підключений.'");
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании();
			РезультатОперации.Результат = Ложь;
			РезультатОперации.ОписаниеОшибки = ОписаниеОшибки;
...
Процедура НачатьЗаписьДанныхВМеткуRFID_ВыполнитьКомандуЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	ОписаниеОшибки = ""; 
	Если НЕ РезультатВыполнения.Результат Тогда
		ОписаниеОшибки = НСтр("ru='При работе со считывателем RFID произошла ошибка: %ОписаниеОшибки%';uk='При роботі зі зчитувачем RFID відбулася помилка: %ОписаниеОшибки%'");
		ОписаниеОшибки = СтрЗаменить(ОписаниеОшибки, "%ОписаниеОшибки%", РезультатВыполнения.ВыходныеПараметры[1]);
	КонецЕсли;
	...
	#Иначе
		...
		Для Каждого Файл Из ПомещаемыеФайлы Цикл
			...
			Исключение
				...
				ТекстСообщения =  НСтр("ru='При чтении файла %ИмяФайла% произошла ошибка';uk='При читанні файлу %ИмяФайла% відбулася помилка'");
...
Процедура НачатьВыполнениеКоманды_ВыбратьСменуЗавершение(СтруктураПараметровСмены, Параметры) Экспорт
	...
	Если СтруктураПараметровСмены = Неопределено Тогда
		Если Параметры.ОповещениеПриЗавершении <> Неопределено Тогда
			ОписаниеОшибки = НСтр("ru='Кассовая смена не выбрана';uk='Касова зміна не вибрана'");
			РезультатОперации = ПараметрыВыполненияОперацииНаОборудовании(Ложь, ОписаниеОшибки);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПриЗавершении, РезультатОперации);
		КонецЕсли;
	Иначе
...
#Область ПроцедурыИФункцииДляРаботыОборудованияПоСтандарту
...
Функция DataEventHandler(Источник, Событие, Данные, ДополнительныеДанные = Неопределено) Экспорт
	...
	// Для Нового стандарта: Изменяем "сырые" данные на данные без префиксов и постфиксов
	Если ДопДанные[1] <> Неопределено Тогда
		ДопДанные[1] = Данные;
	КонецЕсли;
...
//   Buttons  - строка с перечнем кнопок, доступных пользователю в виде "Да+Нет" (должны соответствовать возможностям диалогов программы)
...
// Функция предназначена для перепаковки массивов программы в массивы SafeArray
