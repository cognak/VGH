...
#Иначе
	// Устройство ВГХ
	Если ОбработчикДрайвера = ПредопределенноеЗначение("Перечисление.ОбработчикиДрайверовПодключаемогоОборудования.ОбработчикУстройствоВГХ") Тогда
		Возврат ПодключаемоеОборудованиеУстройствоВГХ;
	Иначе
		Результат = ПодключаемоеОборудованиеУниверсальныйДрайверКлиент;   //УниверсальныйДрайверКлиент;
