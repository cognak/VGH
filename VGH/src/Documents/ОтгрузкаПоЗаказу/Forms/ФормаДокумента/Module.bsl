			...
			Если Результат.Свойство("success") И Результат.success[0].status="OK" Тогда
				...
				Если СтруктураОтвет.Отработало Тогда
					...
					ИначеЕсли ДанныеОтвета.Свойство("success") И ДанныеОтвета.success[0].status="OK" Тогда
						...
						СохранитьРеестр(ДанныеОтвета.success[0]);
	...
	#Иначе
		...
		Если ЗначениеЗаполнено(объект.НомерТТН) Тогда
			...
			Если ЗначениеЗаполнено(Объект[ИмяРеквизита]) Тогда
				...
				Оповещение= Новый ОписаниеОповещения("ПечатьДокументаПоссылкеЗавершение", ЭтотОбъект,	Новый Структура("ИмяКоманды,СсылкаНаДокумент,MP,НомерДокумента",ИмяКоманды, Объект[ИмяРеквизита],Объект.MP,Объект.order));
...
&НаСервере
Процедура СохранитьРеестр(СтруктураРеестра)
	
	Документы.РеестрОтгрузок.СохранитьРеестр(СтруктураРеестра);
