			...
			Если СтруктураОтвет.Отработало Тогда
				...
				Если Результат.Свойство("success") тогда
					Если Результат.success[0].status="OK" Тогда
						...
						Розница = СтруктураОтвет.ДанныеОтвета.success[0].Свойство("roznica");
						...
						ПечатьДокументаПоСсылкеЗавершение(
							Новый Структура("Значение",КодВозвратаДиалога.Да),
							Новый Структура("ИмяКоманды,СсылкаНаДокумент,MP,Розница",
								ИмяКоманды,
								СсылкаНаДокументПечати,
								Объект.MP,
								Розница));
					Иначе 
						текстОшибки=СтруктураОтвет.ДанныеОтвета.info.message;
