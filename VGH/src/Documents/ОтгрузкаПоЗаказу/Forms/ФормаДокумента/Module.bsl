...
Функция ПроверитьНомерНаСайте(Номер)
	...
	вхПараметры=Новый Структура("ИмяМетода,Order","parcel",Номер);
...
Процедура ОтправитьИПолучитьТТН(Команда)
	...
	Если СтруктураОтвет.Отработало Тогда
		...
		ОтправитьВГХНаСайт(объект.order,ДанныеВГХ,СтруктураОтвет);
		// анализируем ответ
		Если СтруктураОтвет.Отработало Тогда
...
&НаСервере
Процедура ОтправитьВГХНаСайт(order,ДанныеВГХ,СтруктураОтвет)
			...
			Если СтруктураОтвет.Отработало Тогда
				...
				Если Результат.Свойство("success") тогда
					Если Результат.success[0].status="OK" Тогда
						...
						//Оповещение= Новый ОписаниеОповещения("ПечатьДокументаПоссылкеЗавершение", ЭтотОбъект,	Новый Структура("СсылкаНаДокумент", СсылкаНаДокументПечати));
						ПечатьДокументаПоСсылкеЗавершение(Новый Структура("Значение",КодВозвратаДиалога.Да),Новый Структура("ИмяКоманды,СсылкаНаДокумент,MP",ИмяКоманды,СсылкаНаДокументПечати,Объект.MP));
					Иначе 
						текстОшибки=СтруктураОтвет.ДанныеОтвета.info.message;
						А = Новый Массив();
						А.Добавить("Ошибка: ");
...
Процедура ПечатьДокументаПоСсылкеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	врФайлПечатнойФормы=вгхОбщийПроцедурыФункцииСервер.ПолучитьФайлССервера(СсылкаНаДокумент,ДополнительныеПараметры.Свойство("MP"));
