...
Процедура ПриОткрытии(Отказ)
	...
	//ЗаполнитьНаименованиеРабочегоМеста(Объект, ТекущийПользователь);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ПроверкаУникальностиПоИдентификаторуКлиента()Тогда
		Отказ = Истина;
		Текст = НСтр("ru='Ошибка сохранения рабочего места."
"Рабочее место с таким идентификатором клиента уже существует.';uk='Помилка збереження робочого місця."
...
"Можливо надалі це ускладнить ідентифікацію та вибір робочого місця."
...
Процедура ПередЗаписьюЗавершение(Результат, Параметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		ОтветПередЗаписью = Истина;  // переменная модуля формы на клиенте
		Записать();
	КонецЕсли;  
