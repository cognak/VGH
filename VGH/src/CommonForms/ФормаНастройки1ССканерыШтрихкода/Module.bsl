...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru='Оборудование:';uk='Обладнання:'") + Символы.НПП  + Строка(Идентификатор);
...
Процедура ОбновитьИнформациюОДрайвере()
	...
	Если МенеджерОборудованияКлиент.ВыполнитьДополнительнуюКоманду("ПолучитьВерсиюДрайвера",
		...
		Если ВерсияИзБПОСтр > ВерсияСтр Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Установленная на компьютере версия драйвера устарела. Необходимо обновление до версии:';uk='Встановлена на комп`ютері версія драйвера застаріла. Потрібне оновлення до версії:'") + Символы.НПП + ВерсияИзБПО);
		КонецЕсли;
	...
	Элементы.ТестУстройства.Доступность = (Драйвер = НСтр("ru='Установлен';uk='Встановлений'"));
